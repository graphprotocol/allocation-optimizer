@testset "costbenefit.jl" begin
    ...
    fake_repository = Repository(
        [
            Indexer(
                "1x000", 5.0, 0.0, [Allocation("0x010", 2.5, alloc_lifetime), Allocation("0x011", 2.5, alloc_lifetime)]
            ),
            Indexer(
                "1x001", 10.0, 0.0, [Allocation("0x010", 2.0, alloc_lifetime), Allocation("0x011", 8.0, alloc_lifetime)]
            ),
        ],
        [Subgraph("0x010", 10.0), Subgraph("0x011", 5.0)],
    )
    network = Network("1", 100.0, 1.0001, 15, 15.0, 3)
    ...
    compared = compare_rewards("1x001", optimized_filterd_repo, fake_repository, network, allocs_list, alloc_lifetime, gas, 1.0)
    @test sum(compared) >= 0
